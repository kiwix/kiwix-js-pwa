...
params['appVersion'] = "2.3.5"; //DEV: Manually update this version when there is a new release: it is compared to the Settings Store "appVersion" in order to show first-time info, and the cookie is updated in app.js
...
params['PWAInstalled'] = getSetting('PWAInstalled') || window.matchMedia('(display-mode: standalone)').matches; // Because user may reset the app, we need another way to know if the app is installed or not, hence we test for standalone mode
