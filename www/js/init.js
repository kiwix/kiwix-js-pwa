...
params['appVersion'] = "2.2.0-WikiMed"; //DEV: Manually update this version when there is a new release: it is compared to the Settings Store "appVersion" in order to show first-time info, and the cookie is updated in app.js
/******* UPDATE THIS ^^^^^^ IN service worker AND PWA-SERVER BELOW !! ********************/
params['packagedFile'] = getSetting('packagedFile') || "mdwiki_en_all-app_maxi_2022-11.zimaa"; //For packaged Kiwix JS (e.g. with Wikivoyage file), set this to the filename (for split files, give the first chunk *.zimaa) and place file(s) in default storage
params['archivePath'] = "archives"; //The directory containing the packaged archive(s) (relative to app's root directory)  
params['fileVersion'] = getSetting('fileVersion') || "mdwiki_en_all-app_maxi_2022-11.zimaa (3 November 2022)"; //This will be displayed in the app - optionally include date of ZIM file
...
params['kiwixDownloadLink'] = "https://download.kiwix.org/zim/.hidden/custom_apps/"; //Include final slash
...
params['contentInjectionMode'] = getSetting('contentInjectionMode') || (navigator.serviceWorker && !/^(ms-appx-web:)$/i.test(window.location.protocol) && !window.nw ? 'serviceworker' : 'jquery'); // Deafault to SW mode if the browser supports it natively and without workarounds
