...
* FEATURE: The Electron and NW.js apps now identify themselves to Kiwix's servers with a User-Agent such as `kiwix/3.9.1 (js-electron-windows)`, or `(js-electron-windows_wikimed)` for a packaged flavour, sent only on requests to *.kiwix.org, so that their traffic can be told apart in Kiwix's statistics (kiwix/operations#797)
