...
function Main {
    ...
    if ((Test-Path $filename -PathType leaf) -and ($filename -imatch '(.*)\.(?:appx|appxbundle|appxupload)$')) {
        ...
        $keyfile = "$PSScriptRoot\ssh_key" -ireplace '[\\/]', '/'
        if ($dryrun) {
            "scp -v -i $keyfile $filename ci@download.kiwix.org:$target"
            "ssh -i $keyfile ci@download.kiwix.org mv $target/$file $target/$newfilename"  
            "Aborting because this is a dry run."
            exit
        }
        # Uploading file
        # Move-Item $filename $originalpath/$newfilename
        & scp @('-i', "$keyfile", "$filename", "ci@download.kiwix.org:$target")
        & ssh @('-i', "$keyfile", 'ci@download.kiwix.org', "mv $target/$file $target/$newfilename")
        "Done."
    } else {
