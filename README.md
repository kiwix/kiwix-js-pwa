...
When the app is running, the Home tab displays a localhost URL (by default `http://localhost:3000`) that you can open in any web browser. You can also enable **Allow external network access** to make the application available to other devices on your local network. The listening port can be changed in **Configuration → Expert Settings**.
...
- External network access is disabled by default, applies only to the current session, and accepts connections only from the local network.
...
Installable and portable versions for Windows (XP/Vista/7/8/10/11), Linux (32bit and 64bit) and macOS (x64/M1/M2/M3 on 12+, and a
legacy build for 10.13-11) are available from [releases](https://github.com/kiwix/kiwix-js-pwa/releases/). Unstable
