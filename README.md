...
It can also be installed (in Windows 10/11) by opening a Command Prompt or PowerShell terminal and typing `winget install kiwix.wikivoyage`. The easiest way to sideload
the UWP app is with `winget install kiwix.wikivoyage`, but alternative sideloading instructions are available in the [release notes](https://kiwix.github.io/kiwix-js-windows/wikivoyage-uwp.html).
